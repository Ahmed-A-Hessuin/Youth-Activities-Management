...
import style from "./detailsActivity.module.css";
...
export default function DetailsActivity({props}) {
...
// /> }
  ...
  async function getDetailsActivity() {
    ...
    let  {data}  = await axios.get(
      `https://actitvityv1.onrender.com/categories/6407a4aa04c364c71a43260b/activities/${currentId}`
    );
...
/>
      ...
      <h2 className={style.h2}>{detailsActivity.title_ar}</h2>
     <div className={style.line}></div>
     <p className={style.createdAt}>{date}</p>

      <p className={style.p}>{detailsActivity.description_ar}</p>
    </div>
    </>
  );
}
